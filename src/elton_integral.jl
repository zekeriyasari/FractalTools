...
function elton_integral(func, ind_func, points::Channel, func_params=tuple(), ind_params=tuple(), ϵ::Real=1e-10, max_iter=1e6, chunk_size = 10)
    number_of_points = 0
...
function elton_integral(func, points::Channel, func_params=tuple(), ϵ::Real=1e-10, max_iter=1e6, chunk_size = 10)
    number_of_points = 0
    old_number_of_points = 0
    result = 0
    Δ = 1e15 * ones(chunk_size)
