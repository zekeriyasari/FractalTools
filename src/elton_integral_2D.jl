...
export elton_integral_2D
#ToDo: Assume that indicator is 1 for one version
# ToDo : Elton integral function and norm function must run vector.


function elton_integral_2D(func, ind_func, points::Channel, func_params=tuple(), ind_params=tuple(), ϵ::Real=1e-10, max_iter=1e6, chunk_size = 10)
    number_of_points = 0
    ...
    # while number_of_points ≤ max_iter
        ...
        filtered_set_of_points = reshape(set_of_points[set_of_points .* indicator.>0],  size(set_of_points,1),number_of_indicator)
...
function elton_integral_2D(func, points::Channel, func_params=tuple(), ϵ::Real=1e-10, max_iter=1e6, chunk_size = 10)
    number_of_points = 0
    old_number_of_points = 0
    result = 0
    Δ = 1e15 * ones(chunk_size)
